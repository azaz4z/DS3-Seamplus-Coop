...
int main() {
    std::cout << "[TEST] Starting unit tests for CountersExtension..." << std::endl;

    // 1. Instance and initialization
    auto ext = std::make_shared<CountersExtension>();
    assert(std::string(ext->GetId()) == "counters");
    assert(std::string(ext->GetName()) == "Counters");
    assert(ext->Initialize());

    // Reset before test to isolate environment
    ext->ResetStats();
    CountersSnapshot initialSnap = ext->GetStats();
    ...
    CountersSnapshot snap = ext->GetStats();
    ...
    // 5. Test disk persistence
    ext->Shutdown(); // Forces save to ds3sc_stats.ini under [COUNTERS]

    CountersExtension ext2;
    assert(ext2.Initialize());
    CountersSnapshot loadedSnap = ext2.GetStats();
    ...
    ext2.ResetStats();
    CountersSnapshot resetSnap = ext2.GetStats();
    ...
    // Increment again then reset via C API
    ext2.RecordDeath();
