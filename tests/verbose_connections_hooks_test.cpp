...
int wmain(int argc, wchar_t** argv) {
    ...
    assert(module);
    auto game = LoadLibraryExW(argv[2], nullptr, DONT_RESOLVE_DLL_REFERENCES);
    assert(game);
    ...
    for (auto* hook : g_bannerHooks) assert(hook);
    ...
    assert(Matches(game, kBannerUpdateRva, kBannerUpdatePrefix, sizeof(kBannerUpdatePrefix)));
    assert(Matches(game, kBannerNextRva, kBannerNextPrefix, sizeof(kBannerNextPrefix)));
    assert(Matches(game, kBannerTimingRva, kBannerDefaults, sizeof(kBannerDefaults)));
    FreeLibrary(game);
    FreeLibrary(module);
    std::puts("PASS: real MinHook installation, reinitialization and byte restoration on ds3sc.dll and DarkSoulsIII.exe");
}
